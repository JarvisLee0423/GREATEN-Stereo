...
from models.greaten_stereo.feature_extractors import Mobilenetv2Encoder, MultiDINOEncoder, LearnableScaleShift, GateMaskGenerator, GatedContextGeometricFusion, GatedContextGeometricDINOFusion, SurfaceNormalSimpleEncoder
...
class GREATENStereo(nn.Module):
    def __init__(self, args: argparse.Namespace):
        ...
        self.cnet = MultiDINOEncoder(
            out_channels=[args.channels, context_channels],
            vit_channels=dim_list_config // 2,
            norm_fn="instance",
        ...
        if self.args.infer_normal:
            ...
            self.gated_context_geometric_fusion_4x = GatedContextGeometricDINOFusion(feat_channels[0], dim_list_config // 2)
            ...
            self.learnable_scale_shift = LearnableScaleShift(args)
    ...
    def infer_mono(self, left_img: torch.Tensor, right_img: torch.Tensor) -> Tuple[torch.Tensor, List[Union[list, torch.Tensor]]]:
        ...
        feat_mono_left, depth_mono_left = self.mono_decoder(feat_left_encoder, patch_h, patch_w, return_feat=True)
        feat_mono_left = F.interpolate(feat_mono_left, size=(height_ori // 4, width_ori // 4), mode="bilinear", align_corners=False)
        ...
        feat_mono_right, depth_mono_right = self.mono_decoder(feat_right_encoder, patch_h, patch_w, return_feat=True)
        feat_mono_right = F.interpolate(feat_mono_right, size=(height_ori // 4, width_ori // 4), mode="bilinear", align_corners=False)
        ...
        return depth_mono_left, depth_mono_right, feat_mono_left, feat_mono_right
    ...
    def forward(self, left_img: torch.Tensor, right_img: torch.Tensor, iters: int=12, disp_init: torch.Tensor=None, disp_gt: torch.Tensor=None, test_mode: bool=False) -> Tuple[Union[torch.Tensor, Union[List[torch.Tensor], dict, int, float]]]:
        ...
        if self.args.infer_normal:
            depth_mono_left, depth_mono_right, feat_mono_left, feat_mono_right = self.infer_mono(left_img, right_img)
        ...
        with autocast(enabled=self.args.mixed_precision, dtype=getattr(torch, self.args.precision_dtype, torch.float16)):
            ...
            if self.args.infer_normal:
                ...
                context_geometric_left_4x, gated_context_left_4x = self.gated_context_geometric_fusion_4x(feat_left[0], normals_feat_left[0], feat_mono_left, gate_mask_left[0])
                ...
                context_geometric_right_4x, gated_context_right_4x = self.gated_context_geometric_fusion_4x(feat_right[0], normals_feat_right[0], feat_mono_right, gate_mask_right[0])
            ...
            if self.args.infer_normal:
                cnet_list = self.cnet(st_aug_left_img, gate_mask_left_4x, feat_mono_left, num_layers=self.args.n_gru_layers)
                net_list = [torch.tanh(self.net_stem[i](torch.cat([x[0], normals_feat_left[i], context_geometric_left[i]], dim=1))) for i, x in enumerate(cnet_list)]
                inp_list = [torch.relu(self.inp_stem[i](torch.cat([x[1], normals_feat_left[i], context_geometric_left[i]], dim=1))) for i, x in enumerate(cnet_list)]
            else:
        ...
        if self.args.infer_normal:
            scale, shift = self.learnable_scale_shift(cv_fn.global_volume_pyramid[0], cv_fn.local_volume_pyramid[0], init_disp, depth_mono_left_4x, coords)
            disp = scale * depth_mono_left_4x + shift
            init_metric_disp = (scale * depth_mono_left + shift) * 4.0
        else:
            disp = init_disp
        disp_preds = []
        ...
        if test_mode:
            if self.args.infer_normal:
                return (init_disp, init_metric_disp), disp_up, ({"apc": cv_fn.local_volume_pyramid, "ccv": cv_fn.global_volume_pyramid}, feat_batch, feat_h, feat_w, [st_aug_left_img_vis, normals_left, normals_left_4x, gate_mask_left_4x], [st_aug_right_img_vis, normals_right, normals_right_4x, gate_mask_right_4x])
            else:
        ...
        if self.args.infer_normal:
            return ((init_disp, init_metric_disp), [st_aug_left_img_vis, normals_left, normals_left_4x, gate_mask_left_4x], [st_aug_right_img_vis, normals_right, normals_right_4x, gate_mask_right_4x]), disp_preds
        else:
