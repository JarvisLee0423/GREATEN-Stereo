...
from modules.backbones.depth_anything.dino import DINOv2
from modules.backbones.depth_anything.utils.blocks import _make_scratch, FeatureFusionBlock
from modules.backbones.depth_anything.utils.transform import Resize, PrepareForNet, NormalizeImage


def _make_fusion_block(features: int, use_bn: bool, size: Any=None) -> nn.Module:
...
class DPTHead(nn.Module):
    ...
    def forward(self, out_features: List[Union[torch.Tensor, List[torch.Tensor]]], patch_h: int, patch_w: int, return_feat: bool=False) -> torch.Tensor:
        out = []
...
class DPTDecoder(nn.Module):
    ...
    def forward(self, out_features: List[Union[torch.Tensor, List[torch.Tensor]]], patch_h: int, patch_w: int) -> tuple:
        out = []
        for i, x in enumerate(out_features):
            if self.use_clstoken:
                x, cls_token = x[0], x[1]
...
class DepthAnythingV2(nn.Module):
    ...
    def forward_features(self, x: torch.Tensor, only_feat: bool=False) -> Union[tuple, torch.Tensor]:
        patch_h, patch_w = x.shape[-2] // 14, x.shape[-1] // 14
    ...
    @torch.no_grad()
    def infer_image(self, raw_image: Union[np.ndarray, torch.Tensor], input_size: int=518) -> np.ndarray:
    ...
    def image2tensor(self, raw_image: Union[np.ndarray, torch.Tensor], input_size: int=518) -> tuple:
        transform = Compose([
            Resize(
                width=input_size,
                height=input_size,
...
class DepthAnythingV2Decoder(nn.Module):
    def __init__(
        self,
        encoder: str="vitl",
        features: int=256,
        ...
        super(DepthAnythingV2Decoder, self).__init__()
        ...
        print("DepthAnythingV2Decoder out_channels: ", out_channels)

        # self.encoder = encoder
        # self.pretrained = DINOv2(model_name=encoder)

        self.depth_head = DPTDecoder(out_channels[-1], features, use_bn, out_channels=out_channels, use_clstoken=use_clstoken)
